...
const Livemap = () => {
  ...
  const [loadingStatus, setLoadingStatus] = useState<"map" | "data" | "ready">(
    "map"
  );
    ...
    layers.forEach((layerId) => {
      ...
      map.current!.on("click", layerId, (e) => {
        if (e.features && e.features.length > 0) {
          ...
          if (props) {
            const train = trainsRef.current.find(
              (t) =>
  ...
  useEffect(() => {
    ...
    if (
      ...
      filteredTrains.length <= 10 &&
      searchQuery.length === 5
    ) {
      const validTrains = filteredTrains.filter(
        (train) =>
          train.current_lat &&
          train.current_lng &&
          typeof train.current_lat === "number" &&
      ...
      if (validTrains.length === 1) {
        ...
        const coordinates = validTrains.map((train) => [
          train.current_lng,
          train.current_lat,
        ]);
    ...
    <div className="relative w-full h-[calc(100vh-4rem)]">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-gradient-to-r from-purple-50 to-purple-100 p-3 rounded-lg border border-purple-200">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Link
                    href={`/dashboard/train?train=${currentTrain.train_number}`}
                ...
                <div className="grid grid-cols-3 gap-2 text-xs text-center">
                  <div className="bg-white/60 p-2 rounded">
                    <div className="text-purple-600 font-medium w-full">
                      Distance
                    </div>
                    <div className="text-purple-900 font-semibold">
                      {formatNumber(currentTrain.curr_distance, 1)} km
                    </div>
                  </div>
                  <div className="bg-white/60 p-2 rounded">
                    <div className="text-purple-600 font-medium w-full">
                    ...
                    <div className="text-purple-900 font-semibold">
                      {getMinutesElapsedLabel(currentTrain.mins_since_dep)}
                    </div>
                  </div>
                  <div className="bg-white/60 p-2 rounded">
                    <div className="text-purple-600 font-medium w-full">
                    ...
                    <div className="text-purple-900 font-semibold">
                      {typeof currentTrain.current_day === "number"
                        ? currentTrain.current_day
                        : "—"}
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-gradient-to-r from-green-50 to-orange-50 p-3 rounded-lg border border-gray-200">
                ...
                <div className="mb-3 pb-3 border-b border-gray-200">
                  ...
                  <div className="ml-6">
                    <div className="text-sm font-semibold text-gray-900">
                      {currentTrain.current_station_name}
                    </div>
                    <div className="text-xs text-gray-600 font-mono">
                      {currentTrain.current_station}
                    </div>
                    <div className="mt-1 text-xs text-green-700">
                      Crossed at:{" "}
                      <span className="font-semibold">
                        {formatMinutesAsClock(currentTrain.departure_minutes)}
                      </span>
                    </div>
                    {typeof currentTrain.halt_mins === "number" &&
                      currentTrain.halt_mins > 0 && (
                        <div className="mt-1 text-xs text-green-700">
                          Halt:{" "}
                          <span className="font-semibold">
                            {currentTrain.halt_mins} mins
                          </span>
                        </div>
                      )}
                  </div>
                ...
                <div>
                  ...
                  <div className="ml-6">
                    <div className="text-sm font-semibold text-gray-900">
                      {currentTrain.next_station_name}
                    </div>
                    <div className="text-xs text-gray-600 font-mono">
                      {currentTrain.next_station}
                    </div>
                    <div className="mt-1 flex items-center gap-3 text-xs">
                      <div className="text-gray-600">
                        Distance:{" "}
                        <span className="font-semibold">
                          {getDistanceToNextKm(currentTrain)} km
                        </span>
                      </div>
                      <div className="text-gray-600">
                        ETA:{" "}
                        <span className="font-semibold">
                          {formatMinutesAsClock(
                            currentTrain.next_arrival_minutes
                          )}
                        </span>
                      </div>
                    </div>
      ...
      {loading && (
        <div className="absolute inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-20">
          <div className="bg-white/90 backdrop-blur rounded-xl shadow-lg px-8 py-6 min-w-[220px] border border-gray-100">
            {/* Loader */}
            <div className="flex justify-center">
              <div className="h-10 w-10 rounded-full border-2 border-gray-300 border-t-gray-800 animate-spin"></div>
            </div>
